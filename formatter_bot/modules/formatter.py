...
async def handle_media(update, context):
    ...
    if msg.photo:
        ...
        await msg.reply_text(f"✅ تم استلام صورة
📦 العدد الحالي: {len(s['inputs'])}")
        ...
        if mime.startswith("video") or ext in video_exts:
            ...
            await msg.reply_text(f"✅ تم استلام فيديو كملف
📦 العدد الحالي: {len(s['inputs'])}")
        elif mime.startswith("image") or ext in image_exts:
            kind = "photo_doc"
            s["inputs"].append((kind, p))
            await msg.reply_text(f"✅ تم استلام صورة كملف
📦 العدد الحالي: {len(s['inputs'])}")
        else:
            try:
                os.remove(p)
        ...
        await msg.reply_text(f"✅ تم استلام فيديو
📦 العدد الحالي: {len(s['inputs'])}")
