...
async def _set_progress_text(message, current_step, total_steps, label, force=False):
    ...
    meta = getattr(message, "_progress_meta", None)
    now = time.monotonic()
    if meta is None:
        meta = {"text": None, "ts": 0.0}
        setattr(message, "_progress_meta", meta)

    if not force:
        same_text = (meta.get("text") == text)
        too_soon = (now - float(meta.get("ts", 0.0))) < 0.7
        if same_text or too_soon:
            return
    ...
    try:
        ...
        msg = str(e).lower()
        if 'retryafter' in name or 'flood' in msg or 'too many requests' in msg:
...
async def finish_custom(update, context):
    ...
    try:
        setattr(progress_msg, "_progress_meta", {"text": None, "ts": 0.0})
    except Exception:
        pass
    ...
    normalized_inputs.sort(key=lambda x: (0 if x[0].startswith("photo") else 1, str(x[2]).lower()))
    ...
    try:
        for index, (kind, path, original_name) in enumerate(normalized_inputs, start=1):
            ...
            step += 1
            await _set_progress_text(progress_msg, step, total_steps, label, force=True)
                ...
                try:
                    ...
                    for fh in handles:
                        try:
                            ...
                            pass
                step += len(chunk)
                await _set_progress_text(progress_msg, step, total_steps, "جاري الإرسال...", force=True)
            photo_batch = []
        ...
        await _set_progress_text(progress_msg, total_steps, total_steps, "اكتملت العملية", force=True)
