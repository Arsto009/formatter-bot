...
def _build_video_tone_filter(brightness_value=0):
    ...
    amount = max(-100, min(100, int(brightness_value)))
    brightness = amount / 100

    # تشبع مرتبط بنسبة الإنارة للفيديو فقط
    saturation = 1 + (amount / 100) * 0.35
    saturation = max(0.55, min(1.60, saturation))

    # تظليل/عمق بسيط يناسب الفيديو ويحافظ على المناطق الداكنة
    contrast = 1 + (amount / 100) * 0.12
    contrast = max(0.85, min(1.25, contrast))

    gamma = 1 - (amount / 100) * 0.08
    gamma = max(0.85, min(1.15, gamma))

    return f"eq=brightness={brightness}:saturation={saturation}:contrast={contrast}:gamma={gamma}"
