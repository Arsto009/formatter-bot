...
async def handle_media(update, context):
    ...
    if msg.video:
        try:
            ...
            ext = os.path.splitext(getattr(msg.video, "file_name", "") or ".mp4")[-1].lower() or ".mp4"
            p = tempfile.mktemp(suffix=ext)
            await f.download_to_drive(p)
            s["inputs"].append(("video", p))
        except Exception as e:
            print("VIDEO ERROR:", e)
            await msg.reply_text("⚠️ الفيديو كبير، أرسله كملف.")
        return
    ...
    if video_group:
        for i in range(0, len(video_group), 10):
            try:
                ...
                print("Video group send error:", e)
                for media in video_group[i:i+10]:
                    try:
                        await q.message.reply_video(media.media, supports_streaming=True)
                    except Exception:
                        try:
                            media.media.seek(0)
                        except Exception:
                            pass
                        await q.message.reply_document(media.media)

    for vdf in video_doc_files:
        try:
            name = getattr(vdf, "name", "").lower()
            if name.endswith(".mov"):
                await q.message.reply_document(vdf)
            else:
                await q.message.reply_document(vdf)
        except Exception as e:
            print("Video doc send error:", e)
