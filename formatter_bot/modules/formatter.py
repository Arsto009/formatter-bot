...
def speed_kb():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("📷 4K احترافي", callback_data="ai:strong")]
    ])

def after_done():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("🔁 المزيد", callback_data="custom:more")],
        [InlineKeyboardButton("⛔ إنهاء", callback_data="custom:end")]
    ])

def send_done():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("📤 تم الإرسال", callback_data="custom:finish")]
    ])

def main_keyboard(user_id=None):
    buttons = [
        [InlineKeyboardButton("🎨 صمم صورتك", callback_data="custom:start")],
        [InlineKeyboardButton("🗑 مسح الإعدادات", callback_data="custom:clear_settings")]
    ]
