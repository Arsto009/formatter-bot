...
from telegram import InlineKeyboardButton, InlineKeyboardMarkup, InputMediaPhoto, InputMediaDocument, InputFile
...
async def handle_media(update, context):
    ...
    try:
        ...
        if msg.photo:
            ...
            s["inputs"].append((kind, p, msg.document.file_name or os.path.basename(p)))
            if kind == "video_doc":
                await _update_receive_counter(
                    msg,
                    s,
            ...
            s["inputs"].append(("video", p, "video_note.mp4"))
            await _update_receive_counter(
                msg,
                s,
                f"✅ تم استلام فيديو\n"
            ...
            p = await _download_media_to_temp(f, suffix=".mp4")
            s["inputs"].append(("video", p, msg.video.file_name or "video.mp4"))
            await _update_receive_counter(
                msg,
                s,
                f"✅ تم استلام فيديو\n"
...
async def finish_custom(update, context):
    ...
    async def process_item(kind, path, original_name=None):
        if kind.startswith("photo"):
            ...
            if kind == "video_doc":
                video_doc_files.append((open(outv, "rb"), original_name or os.path.basename(outv)))
            else:
                video_files.append((open(outv, "rb"), original_name or os.path.basename(outv)))
    ...
    for i, item in enumerate(ordered_inputs, start=1):
        if len(item) == 3:
            kind, path, original_name = item
        else:
            kind, path = item
            original_name = None
        ...
        await process_item(kind, path, original_name)
    ...
    for vf, original_name in video_files:
        try:
            try:
                vf.seek(0)
            except Exception:
            ...
            if hasattr(vf, "name") and os.path.exists(vf.name) and os.path.getsize(vf.name) > 45_000_000:
                await q.message.reply_document(InputFile(vf, filename=original_name or os.path.basename(getattr(vf, "name", "video.mp4"))))
            else:
    ...
    for vdf, original_name in video_doc_files:
        try:
            vdf.seek(0)
        except Exception:
            pass
        await q.message.reply_document(InputFile(vdf, filename=original_name or os.path.basename(getattr(vdf, "name", "video.mp4"))))
