...
def _build_video_tone_filter(brightness_value=0):
    ...
    # سطوع حقيقي بدون ضباب، مع رفع تشبع وتباين بشكل متوازن للفيديو فقط
    brightness = value / 120.0
    contrast = 1.0 + (abs(value) / 220.0)
    saturation = 1.0 + (max(0, value) / 55.0)

    # عند التعتيم لا نرفع التشبع، فقط نقلله قليلًا حتى لا تصبح الألوان متسخة
    if value < 0:
        saturation = max(0.7, 1.0 + (value / 180.0))

    contrast = max(0.9, min(1.6, contrast))
    saturation = max(0.7, min(2.4, saturation))
