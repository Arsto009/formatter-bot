...
def apply_custom_logo_video(video_path, logo_path, width_ratio, opacity, brightness_value=0):
    ...
    if brightness_value:
        filter_complex = (
            f"[1:v]format=rgba,colorchannelmixer=aa={opacity/100}[logo_rgba];"
            f"[logo_rgba][0:v]scale2ref=w=main_w*{width_ratio}:h=ow/mdar[logo_scaled][base];"
            f"[base]eq=brightness={brightness}[base_b];"
            f"[base_b][logo_scaled]overlay=(main_w-overlay_w)/2:(main_h-overlay_h)/2[v]"
        )
    else:
        filter_complex = (
            f"[1:v]format=rgba,colorchannelmixer=aa={opacity/100}[logo_rgba];"
            f"[logo_rgba][0:v]scale2ref=w=main_w*{width_ratio}:h=ow/mdar[logo_scaled][base];"
            f"[base][logo_scaled]overlay=(main_w-overlay_w)/2:(main_h-overlay_h)/2[v]"
        )
