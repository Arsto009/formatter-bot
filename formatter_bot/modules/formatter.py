...
MAX_BOT_DOWNLOAD_SIZE = 20 * 1024 * 1024
...
async def handle_media(update, context):
    ...
    if msg.photo:
        ...
        if file_size > MAX_BOT_DOWNLOAD_SIZE:
            await msg.reply_text(
                f"⚠️ هذا الملف حجمه {round(file_size / (1024 * 1024), 1)} MB\n"
                f"والبوت الحالي لا يستطيع تنزيل ملفات أكبر من 20 MB من تيليجرام عبر Bot API."
            )
            return
        ...
        if mime.startswith("video") or ext in video_exts:
            ...
            try:
                ...
                pass
            await msg.reply_text("⚠️ هذا الملف ليس صورة أو فيديو مدعوم")
            return
        ...
        if file_size > MAX_BOT_DOWNLOAD_SIZE:
            await msg.reply_text(
                f"⚠️ هذا الفيديو حجمه {round(file_size / (1024 * 1024), 1)} MB\n"
                f"والبوت الحالي لا يستطيع تنزيل فيديوهات أكبر من 20 MB من تيليجرام عبر Bot API."
            )
            return
...
async def finish_custom(update, context):
    ...
    ordered_inputs = []
    ordered_inputs.extend([item for item in s["inputs"] if item[0] in ("photo", "photo_doc")])
    ordered_inputs.extend([item for item in s["inputs"] if item[0] not in ("photo", "photo_doc")])
    ...
    for i, (kind, path) in enumerate(list(ordered_inputs), start=1):
