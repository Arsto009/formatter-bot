...
from modules.big_media import (
    TELEGRAM_REMOTE_DOWNLOAD_LIMIT,
    get_media_download_warning,
    get_message_media_meta,
    is_local_bot_api_enabled,
)
...
async def handle_media(update, context):
    ...
    media_meta = get_message_media_meta(msg)
    if media_meta and not is_local_bot_api_enabled(context):
        if media_meta["file_size"] > TELEGRAM_REMOTE_DOWNLOAD_LIMIT:
            await msg.reply_text(get_media_download_warning(media_meta["label"], media_meta["file_size"]))
            return
